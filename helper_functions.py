...
def plot_roc_curves(best_a, best_b, best_c, title):
...
def avg_classification_reports(reports, title):
    ...
    sns.heatmap(pd.DataFrame(avg_reports[0]['report']).iloc[:-1, :].T, annot=True, vmin=0, vmax=1, ax=ax1, xticklabels=True, yticklabels=True, cbar=True).set_title(avg_reports[0]['label'])
    sns.heatmap(pd.DataFrame(avg_reports[1]['report']).iloc[:-1, :].T, annot=True, vmin=0, vmax=1, ax=ax2, xticklabels=True, yticklabels=False, cbar=True).set_title(avg_reports[1]['label'])
    sns.heatmap(pd.DataFrame(avg_reports[2]['report']).iloc[:-1, :].T, annot=True, vmin=0, vmax=1, ax=ax3, xticklabels=True, yticklabels=False, cbar=True).set_title(avg_reports[2]['label'])
    plt.suptitle(title)
